...
async def chat(
    ...
    if prompt_language == "ru":
        skill_instructions = (
            ...
            "[АГЕНТСКИЕ — В НАЧАЛЕ ОТВЕТА]\n"
            "Сначала ты отправляешь команду, потом получаешь результат следующим сообщением и продолжаешь ответ уже с ним.\n"
            "Не додумывай результат поиска или веба заранее — сначала вызови навык.\n\n"
            "[SEARCH_MEMORIES: <запрос>] — вспомнить что-то из прошлых разговоров.\n"
            "  • Это агентский навык: используй его в начале, до финального ответа.\n"
            "  • Результаты поиска вернутся тебе следующим сообщением, и ты продолжишь ответ уже с ними.\n"
            ...
            "[ФИНАЛЬНЫЕ — В САМОМ КОНЦЕ ОТВЕТА]\n"
            "Добавляй их только после основного текста. Каждая команда — на отдельной строке.\n\n"
            "[SAVE_MEMORY: <факт>] — запомнить что-то важное о нём/ней.\n"
            "  • Это финальный навык: добавляй его только в самом конце уже готового ответа.\n"
        ...
        skill_instructions = (
            ...
            "[AGENTIC — AT THE START OF THE REPLY]\n"
            "First you send the command, then you receive the result in the next message, and continue the reply using it.\n"
            "Do not make up search/web results in advance — call the skill first.\n\n"
            "[SEARCH_MEMORIES: <query>] — recall something from past conversations.\n"
            "  • This is an agentic skill: use it at the start, before the final reply.\n"
            "  • Search results will come back in the next message, and you should continue the reply using them.\n"
            ...
            "[FINAL — AT THE VERY END OF THE REPLY]\n"
            "Add these only after the main text. Each command must be on its own line.\n\n"
            "[SAVE_MEMORY: <fact>] — remember something important about them.\n"
            "  • This is a final skill: add it only at the very end of a completed reply.\n"
